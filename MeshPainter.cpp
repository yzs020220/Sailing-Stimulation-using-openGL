...
std::vector<int> MeshPainter::getMeshTypes() { return mesh_types; };
...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader){
	...
	object.typeLocation = glGetUniformLocation(object.program, "type");
...
void MeshPainter::addMesh( TriMesh* mesh, const std::string &name, const std::string &texture_image, const std::string &vshader, const std::string &fshader, int type){
	mesh_names.push_back(name);
    meshes.push_back(mesh);
    mesh_types.push_back(type);
...
void MeshPainter::replaceMesh(TriMesh* mesh, const std::string& name, const std::string& texture_image, const std::string& vshader, const std::string& fshader, int type) {
...
{    
	...
	// 将着色器 type 设置为0，表示使用Phong着色，如果是1使用贴图
	glUniform1i(object.typeLocation, type);
   ...
   //  glUniform1i(glGetUniformLocation(object.program, "texture"), 0);
    ...
    if (type == 3)
    {
        ...
        glUniform1i(object.typeLocation, 5);
        // 传递 unifrom 关键字的矩阵数据。
        glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
        glUniformMatrix4fv(object.viewLocation, 1, GL_TRUE, &camera->viewMatrix[0][0]);
        glUniformMatrix4fv(object.projectionLocation, 1, GL_TRUE, &camera->projMatrix[0][0]);
...
void MeshPainter::drawMeshes(Light *light, Camera* camera){
    ...
    {
        drawMesh(meshes[i], opengl_objects[i], light, camera, mesh_types[i]);
    }
};
